...
class DataExportController:
    ...
    def _mds_create(self):
        ...
        response = [
                    ...
                    header[8], header[9], 0x00, 0x01, 0x00, 0x0E, # RORSapdu
                    ]
    ...
    def _parse_attr(self, result, index):
        attr_id = result[index] << 8 | result[index + 1]
        length = result[index + 2] << 8 | result[index + 3]
        data = result[index+4:index+4+length]
        if attr_id == 0: # TODO: Parse these attributes
            pass
        else:
            data = (None,)
        return (data, length)
        ...
        for i in range(count): # Loop through all SingleContextPoll's
            ...
            for j in range(poll_count): # Loop through all ObservationPoll's
                ...
                for k in range(ava_count): # Loop through AVA's
                    attr = _parse_attr(result, index)
                    index += 4 + attr[1]
                    observation_data.append(attr[0])
